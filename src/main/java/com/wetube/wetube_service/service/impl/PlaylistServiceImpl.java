...
public class PlaylistServiceImpl implements PlaylistService {
    ...
    @Override
    public UserResponseDto.PlaylistDetailDto getPlaylistVideoById(UUID playlistId) {
        Playlist playlist = plr.findById(playlistId)
                .orElseThrow(() -> new ResourceNotFoundException("Playlist", "id", playlistId.toString()));
        List<PlaylistVideo> playlistVideos = plvr.findByPlaylistId(playlistId);
       
        List<Video> videos = playlistVideos.stream()
    ...
    public void removePlaylist(UUID playlistId) {
     ...
     @Override
        public List<PlaylistUserDto> getRecentlyAddedPlaylists(UUID userId, int limit) {
            List<Playlist> playlists = plr.findByUser_Id(userId);
            if (playlists.isEmpty()) {
                throw new ResourceNotFoundException("userId", "id", userId.toString());
            }
            ...
            return playlists.stream()
                    .sorted(Comparator.comparing(Playlist::getCreatedAt).reversed()) // mới nhất trước
                    .limit(limit)
